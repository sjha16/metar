...
def fetch_weather_data(icao: str):
    ...
    with st.spinner("🤖 AI analyzing weather patterns..."):
        try:
            # Use cached version for repeated requests
            analysis_json_str = cached_analyze_metar(raw_metar, raw_taf)
            ...
            st.error(f"❌ AI response parse error: {str(e)}")
            st.code(analysis_json_str[:500] if analysis_json_str else "No response")
            return False
